...
def get_ai_recommendation():
    ...
    try:
        ...
        # High spread = agent is confident; low spread = agent is uncertain
        if q_std > 0.5:
            confidence = min(0.97, 0.5 + (q_max - q_mean) / (2 * q_std + 1e-8))
        else:
            confidence = 0.40  # clearly signal low confidence to user
        ...
        # ── If agent is uncertain (low Q-spread), override with rule-based ──
        # This ensures recommendation ALWAYS changes with context
        # even when the model needs more training
        if q_std < 0.5 and not agent_loaded:
            # Fallback: pure rule-based when agent is untrained
            if days_left < 7 and total_load < 0.6:
                action = 0  # drop both prices urgently
            ...
            # ── Rich reason generation based on WHAT the agent sees ──
            reasons = []
        ...
        # ── Top-3 actions for display (so user sees alternatives) ──
        ...
        top3_actions = [
            {
                ...
                'probability': float(np.exp(q_values[i]) / np.sum(np.exp(q_values)))
            }
        ...
        return jsonify({
            ...
            'q_spread': float(q_std),           # NEW: shows if agent is decisive
            'top3_actions': top3_actions,        # NEW: shows alternatives
